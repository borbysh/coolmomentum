# This is a modified version of the original at
...
def build_learning_rate(initial_lr,
                        ...
                        lr_decay_type='cool',  #cool
                        decay_factor=0.97,
                        decay_epochs=2.4,
                        total_steps=None,
                        warmup_epochs=0):  #cool: no warm-up
  """Build learning rate."""
...
def build_optimizer(learning_rate,
                    global_step, #cool
                    optimizer_name='momentum', #cool
                    decay_type='cool', #cool
                    total_steps=None,
                    steps_per_epoch=None,#cool
                    decay=0.9,
                    epsilon=0.001,
                    momentum=0.99,
                    warmup_epochs=5): #cool
  """Build optimizer."""
  local_momentum = momentum
  local_learning_rate = learning_rate
  if decay_type == 'cool' and total_steps is not None :
    alpha = (1.0 - momentum)**(1.0/tf.cast(total_steps, tf.float32))
    alpha_power = alpha**tf.cast(global_step, tf.float32)
    l_m = 1.0 - ((1.0 - momentum)/alpha_power)
    local_momentum = tf.cond(l_m > 0.0, lambda: l_m, lambda: 0.0)

    if warmup_epochs:
      tf.logging.info('Momentum warmup_epochs: %d' % warmup_epochs)
      warmup_steps = int(warmup_epochs * steps_per_epoch)
      warmup_mom = 0.0
      local_momentum = tf.cond(global_step < warmup_steps, lambda: warmup_mom, lambda: local_momentum)



    local_learning_rate = learning_rate * (1.0 + local_momentum)/2.0
  if optimizer_name == 'sgd':
    ...
    optimizer = tf.train.MomentumOptimizer(
        learning_rate=local_learning_rate, momentum=local_momentum) #cool
...
def archive_ckpt(ckpt_eval, ckpt_objective, ckpt_path):
  ...
  if tf.gfile.Exists(os.path.join(ckpt_dir, 'plot.txt')):
    with tf.gfile.GFile(os.path.join(ckpt_dir, 'plot.txt'), 'a') as f:
      f.write('%s' % ckpt_eval) 
  else:
    with tf.gfile.GFile(os.path.join(ckpt_dir, 'plot.txt'), 'w') as f:
      f.write('%s' % ckpt_eval) 
