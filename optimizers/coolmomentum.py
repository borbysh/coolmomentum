...
class Coolmom(Optimizer):
    ...
    def __init__(self, params, lr=0.01, rho_0=0.99, cool_steps=78200, # of simulated annealing iterations
        ...
        self.epoch = None
        self.rho = 0.0
        self.filename='temperature_coolmom(lr={}'.format(lr)+';rho_0={}'.format(rho_0)+';cool_steps={}'.format(cool_steps)+').txt'
        self.period = 391
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                dx = state['dx']
                rho_0 = group['rho_0']
                beta_power = self.beta ** state['step']
                
                self.rho = (1 - (1 - rho_0)/(1 + rho_0)/beta_power)/(1 + (1 - rho_0)/(1 + rho_0)/beta_power)
